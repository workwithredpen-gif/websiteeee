...
lightbox_html = """
  <!-- Lightbox -->
  <div id="lightbox" class="fixed inset-0 z-[100] bg-black/98 hidden flex-col items-center justify-center transition-opacity duration-300 opacity-0 relative">
...
gallery_regex = re.compile(r'<!-- Gallery -->.*?<\/section>', re.DOTALL)

for cat in categories:
    new_content = html_content
    
    new_content = re.sub(r'<title>.*?<\/title>', f"<title>{cat['title']} Photography — RedPen Comms</title>", new_content)
    new_content = re.sub(r'<h1 class="text-4xl font-bold mb-4">.*?<\/h1>', f'<h1 class="text-4xl font-bold mb-4">{cat["title"]}</h1>', new_content)
    new_content = re.sub(r'<p class="text-lg text-gray-600">.*?<\/p>', f'<p class="text-lg text-gray-600 px-4">{cat["desc"]}</p>', new_content, flags=re.DOTALL)
  ...
  <section class="py-16 container mx-auto px-4">
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <!-- Placeholders -->
        <div class="relative group block h-64 overflow-hidden rounded-xl shadow-md bg-gray-200"></div>
        <div class="relative group block h-64 overflow-hidden rounded-xl shadow-md bg-gray-200"></div>
        <div class="relative group block h-64 overflow-hidden rounded-xl shadow-md bg-gray-200"></div>
      </div>
    </div>
    ...
    new_content = gallery_regex.sub(new_gallery, new_content)
    new_content = new_content.replace('</body>', f'{lightbox_html}\n</body>')
